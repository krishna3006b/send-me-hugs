...
export const getRaisings = async (req: Request, res: Response) => {
    try {
        ...
        res.status(200).json({ success: true, raisings });
    } catch (err) {
...
export const getRaising = async (req: Request, res: Response) => {
    try {
        ...
        if (raising) {
            res.status(200).json({ success: true, raising });
        } else {
...
export const updateRaising = async (req: Request, res: Response) => {
    try {
        ...
        const raising = await Raising.findOneAndUpdate({ id: id }, req.body, { new: true });
        if (raising) {
            res.status(200).json({ success: true, message: "Campaign updated successfully", raising });
        } else {
            res.status(404).json({ success: false, message: "Campaign not found" });
        }
    } catch (err) {
        res.status(500).json({ success: false, message: "Failed to update campaign", error: (err instanceof Error) ? err.message : err });
    }
};
...
export const removeRaising = async (req: Request, res: Response) => {
    try {
        const id = req.params.id;
        const raising = await Raising.findOneAndDelete({ id: id });
        if (raising) {
            res.status(200).json({ success: true, message: "Campaign deleted successfully", raising });
        } else {
            res.status(404).json({ success: false, message: "Campaign not found" });
        }
    } catch (err) {
